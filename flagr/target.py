...
if _ENCHANT_AVAILABLE:
    try:
        # Try en_US first, then default locale
        for _lang in ["en_US", "en_GB", "en"]:
            try:
                DICTIONARY = enchant.Dict(_lang)
                break
...
class Target(object):
    ...
    page. Flagr will automatically attempt to fetch the page. In a similar fashion, ``raw`` will return the content of
    ...
    def build_target(self):
        ...
        if "image" in self.magic.lower():
            ...
            #             if self.path:
            #                 flagr.add_image(os.path.abspath(self.path))
            self.is_image = True
        ...
        # results? :?
        # flagr.locate_flags(parent, self.magic)

        all_words = 0
        english_words = 0

        # Hash the target content for comparison to previous
        # targets by Flagr. This prevents recursion on the
        # same target type.
        ...
        with self.stream as st:
            for chunk in iter(lambda: st.read(4096), b""):
                ...
                if DICTIONARY is not None:
                    ...
                    # No dictionary available, skip English detection
                    self.is_english = False
    ...
    def __str__(self):
        ...
        return flagr.util.ellipsize(repr(self), length=64)
