...
import textwrap
try:
    from cmd2 import clipboard as _cmd2_clipboard
except ImportError:
    _cmd2_clipboard = None
...
from flagr.monitor import JsonMonitor
from flagr.repl import ctf
from flagr.target import Target
from flagr.unit import Unit
from flagr.repl.ctf import CTFProvider, Challenge, User
...
class MonitoringEventHandler(FileSystemEventHandler):
    ...
    def __init__(self, repl: "flagr.repl.Repl", *args, **kwargs):
...
class ReplMonitor(JsonMonitor):
    ...
    def on_flag(self, manager: Manager, unit: Unit, flag: str):
        ...
        #         os.path.join(os.path.dirname(flagr.util.__file__), "flagr.png"),
        #     )
        #     notification.show()
        ...
        log_entry += (
            f" {' ' * len(chain)}Flag: {Fore.GREEN}{Style.BRIGHT}{flag}{Style.RESET_ALL} - "
            f"(copied)"
        )
        ...
        try:
            if _cmd2_clipboard is not None:
                _cmd2_clipboard.write_to_paste_buffer(flag)
    ...
    def on_exception(
        self, manager: flagr.manager.Manager, unit: flagr.unit.Unit, exc: Exception
    ) -> None:
    ...
    def on_work(
        self,
        manager: flagr.manager.Manager,
        threadid: int,
        unit: flagr.unit.Unit,
        case: Any,
    ):
    ...
    def on_download_update(
        self, manager: flagr.manager.Manager, download: flagr.manager.Download
    ) -> None:
    ...
    def on_artifact(
        self, manager: flagr.manager.Manager, unit: flagr.unit.Unit, path: str = None
    ) -> None:
...
def get_target_choices(repl, uncomplete=False) -> List[CompletionItem]:
    ...
    result = [
        CompletionItem(t.hash.hexdigest(), flagr.util.ellipsize(repr(t), 40))
        for t in targets
    ]
...
def get_monitor_choices(repl: "flagr.repl.Repl") -> List[CompletionItem]:
    """
...
def get_challenge_choices(repl: "flagr.repl.Repl") -> List[CompletionItem]:
    """
    Get a list of completion items for the possible challenges
    :param repl: a flagr Repl object
...
class Repl(cmd2.Cmd):
    """ A simple Flagr REPL implemented using the cmd2 module.
    ...
    def generate_prompt(self, about_to_wait=False):
        ...
        # update the prompt
        prompt = (
            f"{Fore.CYAN}flagr{Style.RESET_ALL} - {state} - {download_state}"
    ...
    def do_status(self, args):
        ...
        for tid, status in self.manager.monitor.thread_status.items():
            ...
            threads[tid] = (
                f"{str(unit)}",
                f"{flagr.util.ellipsize(repr(target), 50)}",
                f"{flagr.util.ellipsize(repr(case), 20)}",
            )
    ...
    def do_quit(self, args: argparse.Namespace) -> bool:
        """
        Exit the flagr REPL. Optionally, force current evaluation to complete immediately.
    ...
    target_add_parser.add_argument(
        "--flag", "-f", help="Custom flag format for this target (e.g. picoCTF{*})"
    )
    target_add_parser.add_argument(
        "--unit",
        "-u",
    ...
    def _target_add(self, args: argparse.Namespace) -> None:
        ...
        if args.only:
            ...
            try:
                ...
                # The target finished! Show flag status
                flags = [f[1] for f in self.manager.monitor.flags if f[0].origin == target]
                if flags:
                    for f in flags:
                        self.poutput(
                            f"[{Fore.GREEN}+{Style.RESET_ALL}] Flag: "
                            f"{Fore.GREEN}{Style.BRIGHT}{f}{Style.RESET_ALL}"
                        )
                else:
                    self.poutput(
                        f"[{Fore.RED}-{Style.RESET_ALL}] flag unable to be retrieved"
                    )
    ...
    def _target_list(self, args: argparse.Namespace) -> None:
        ...
        for target in targets:
            ...
            # Add flags if there are any, or show not-found message
            if flags:
                output += "\n".join(
                    f" Flag: {Fore.GREEN}{Style.BRIGHT}{f}{Style.RESET_ALL}" for f in flags
                )
            elif target.completed:
                output += f" {Fore.RED}flag unable to be retrieved{Style.RESET_ALL}"
    ...
    def generate_solution(self, flag):
        ...
        log_entry += (
            f" {' ' * len(chain)}Flag: {Fore.GREEN}{Style.BRIGHT}{flag[1]}{Style.RESET_ALL} - "
            f"(copied)"
        )
    ...
    set_parser = Cmd2ArgumentParser(
        description=r"""Set or retreive a flagr runtime parameter. Parameters may be specified as """
    ...
    def do_set(self, args: argparse.Namespace):
        ...
        if args.value and args.parameter is not None:
            # Ensure the section exists
            if section not in self.manager:
                self.manager[section] = {}
            # Set the value
            self.manager[section][name] = args.value
        elif args.value and args.parameter is None:
            self.perror("parameter name required when setting a value")
    ...
    ctf_queue_parser.add_argument("--flag", help="Custom flag format for this target (e.g. picoCTF{*})")
    ctf_queue_parser.add_argument(
        "--unit",
        "-u",
        action="append",
    ...
    def _ctf_queue(self, args: argparse.Namespace) -> None:
        ...
        if args.only:
            ...
            try:
                ...
                # The targets finished! Show flag status
                all_flags = []
                for t in targets:
                    flags = [f[1] for f in self.manager.monitor.flags if f[0].origin == t]
    ...
    # ======================================================================
    # Quality of Life: Challenge Notes
    # ======================================================================

    notes_parser = Cmd2ArgumentParser(
        description="Add, view, or list notes for challenges and targets"
    )
    ...
    notes_add_parser = notes_subparsers.add_parser(
        "add", aliases=["a"], help="Add a note"
    )
    notes_add_parser.add_argument("key", help="Challenge ID or target hash")
    notes_add_parser.add_argument("text", nargs="+", help="Note text")
    notes_add_parser.set_defaults(action="add")

    notes_view_parser = notes_subparsers.add_parser(
        "view", aliases=["v", "show"], help="View notes for a key"
    )
    notes_view_parser.add_argument("key", help="Challenge ID or target hash")
    notes_view_parser.set_defaults(action="view")

    notes_list_parser = notes_subparsers.add_parser(
        "list", aliases=["ls", "l"], help="List all notes"
    )
    notes_list_parser.set_defaults(action="list")

    notes_clear_parser = notes_subparsers.add_parser(
        "clear", help="Clear notes for a key"
    )
    notes_clear_parser.add_argument("key", help="Challenge ID or target hash")
    notes_clear_parser.set_defaults(action="clear")

    @cmd2.with_argparser(notes_parser)
    def do_notes(self, args: argparse.Namespace) -> bool:
        """Manage notes for challenges and targets"""

        if not hasattr(self, "_notes"):
            self._notes: Dict[str, List[str]] = {}

        if args.action == "add":
            if args.key not in self._notes:
                self._notes[args.key] = []
            note = " ".join(args.text)
            self._notes[args.key].append(note)
            self.poutput(
                f"[{Fore.GREEN}+{Style.RESET_ALL}] note added for {args.key}"
            )

        elif args.action == "view":
            if args.key not in self._notes or not self._notes[args.key]:
                self.poutput(
                    f"[{Fore.YELLOW}!{Style.RESET_ALL}] no notes for {args.key}"
                )
            else:
                self.poutput(
                    f"{Fore.CYAN}{args.key}{Style.RESET_ALL} notes:"
                )
                for i, note in enumerate(self._notes[args.key], 1):
                    self.poutput(f"  {i}. {note}")

        elif args.action == "list":
            if not self._notes:
                self.poutput(
                    f"[{Fore.YELLOW}!{Style.RESET_ALL}] no notes saved"
                )
            else:
                for key, notes in self._notes.items():
                    self.poutput(
            ...
            if args.key in self._notes:
                del self._notes[args.key]
                self.poutput(
                    f"[{Fore.GREEN}+{Style.RESET_ALL}] notes cleared for {args.key}"
                )

        return False

    # ======================================================================
    # Quality of Life: Export Solutions / Writeup
    # ======================================================================

    export_parser = Cmd2ArgumentParser(
        description="Export solutions and writeups"
    )
    ...
    def do_export(self, args: argparse.Namespace) -> bool:
        """Export all solutions to a writeup file"""

        flags = self.manager.monitor.flags
        if not flags:
            self.poutput(
                f"[{Fore.YELLOW}!{Style.RESET_ALL}] no flags to export"
            )
            return False
        ...
        if args.format == "markdown":
            lines.append("# CTF Writeup\n")
            lines.append("Generated by flagr\n")
            lines.append("---\n")
            for unit, flag in flags:
                chain = []
                link = unit
                while link is not None:
                    chain.append(link)
                ...
                lines.append(f"**Target:** `{chain[0].target}`\n")
                lines.append("**Solution chain:**\n")
                for i, c in enumerate(chain):
                    lines.append(f"{'  ' * i}{i+1}. `{c}` on `{c.target}`")
                lines.append("")

                if hasattr(self, "_notes"):
                    target_hash = chain[0].target.hash.hexdigest()
                    if target_hash in self._notes:
                        lines.append("**Notes:**\n")
                        for note in self._notes[target_hash]:
                            lines.append(f"- {note}")
                        lines.append("")
                lines.append("---\n")
            ...
            for unit, flag in flags:
                ...
                link = unit
                while link is not None:
                    chain.append(str(link))
                    link = link.target.parent
                chain.reverse()
                data.append({
                    "flag": flag,
                    "target": str(unit.origin),
                    "chain": chain,
                })
            lines.append(json_mod.dumps(data, indent=2))
            ...
            for unit, flag in flags:
                ...
                lines.append(f"Target: {chain[0].target}")
                lines.append(
                    f"Chain: {' -> '.join(str(c) for c in chain)}"
                )
                lines.append("")
        ...
        try:
            with open(args.output_file, "w") as f:
                f.write("\n".join(lines))
            self.poutput(
                f"[{Fore.GREEN}+{Style.RESET_ALL}] exported {len(flags)} "
                f"flag(s) to {args.output_file}"
            )
        except IOError as e:
            self.perror(f"[{Fore.RED}-{Style.RESET_ALL}] write failed: {e}")

        return False

    # ======================================================================
    # Quality of Life: Auto-download CTF files
    # ======================================================================

    download_parser = Cmd2ArgumentParser(
        description="Download all files for a CTF challenge to a local directory"
    )
    download_parser.add_argument(
        "challenge_id", type=str, help="Challenge ID to download files from",
        choices_provider=get_challenge_choices,
    )
    download_parser.add_argument(
        "--dir", "-d", default="./downloads",
        help="Directory to save files to",
        completer=cmd2.Cmd.path_complete,
    )

    @cmd2.with_argparser(download_parser)
    def do_download(self, args: argparse.Namespace) -> bool:
        """Download challenge files to a local directory"""

        if self.ctf_provider is None:
            self.perror(
                f"[{Fore.RED}-{Style.RESET_ALL}] not connected to a CTF"
            )
            return False
        ...
        try:
            ...
            self.perror(
                f"[{Fore.RED}-{Style.RESET_ALL}] invalid challenge id"
            )
            return False

        if not challenge.files:
            self.poutput(
                f"[{Fore.YELLOW}!{Style.RESET_ALL}] no files for this challenge"
            )
            return False

        download_dir = os.path.join(args.dir, challenge.title.replace(" ", "_"))
        os.makedirs(download_dir, exist_ok=True)

        for filename, url in challenge.files.items():
            filepath = os.path.join(download_dir, filename)
            self.poutput(
                f"[{Fore.BLUE}*{Style.RESET_ALL}] downloading {filename}..."
            )
            try:
                r = __import__("requests").get(url, stream=True, timeout=30)
                with open(filepath, "wb") as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        f.write(chunk)
                self.poutput(
                    f"[{Fore.GREEN}+{Style.RESET_ALL}] saved {filepath}"
                )
            except Exception as e:
                self.perror(
                    f"[{Fore.RED}-{Style.RESET_ALL}] download failed: {e}"
                )

        return False

    # ======================================================================
    # Quality of Life: Batch / Parallel target processing
    # ======================================================================

    batch_parser = Cmd2ArgumentParser(
        description="Queue multiple targets at once for parallel processing"
    )
    batch_parser.add_argument(
        "targets", nargs="+", help="Targets to queue (files, URLs, data)",
        completer=cmd2.Cmd.path_complete,
    )
    batch_parser.add_argument(
        "--flag", "-f", help="Custom flag format (e.g. picoCTF{*})"
    )
    batch_parser.add_argument(
        "--unit", "-u", help="Specific unit to run"
    )

    @cmd2.with_argparser(batch_parser)
    def do_batch(self, args: argparse.Namespace) -> bool:
        """Queue multiple targets for parallel processing"""

        config = None
        if args.flag or args.unit:
            config = configparser.ConfigParser()
            config.read_dict(self.manager)
            if args.flag:
                config["manager"]["flag-format"] = args.flag
            if args.unit:
                config["manager"]["units"] = args.unit

        queued = 0
        for target in args.targets:
            self.poutput(
                f"[{Fore.GREEN}+{Style.RESET_ALL}] queuing: {target}"
            )
            self.manager.queue_target(target, config=config, background=True)
            queued += 1

        self.poutput(
            f"[{Fore.GREEN}+{Style.RESET_ALL}] queued {queued} target(s)"
        )
