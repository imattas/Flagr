...
class Monitor(object):
    ...
    def on_data(
        self, manager: flagr.manager.Manager, unit: flagr.unit.Unit, data: Any
    ) -> None:
    ...
    def on_flag(
        self, manager: flagr.manager.Manager, unit: flagr.unit.Unit, flag: str
    ) -> None:
    ...
    def on_manager_exception(
        self, manager: flagr.manager.Manager, exc: Exception
    ) -> None:
    ...
    def on_completion(self, manager: flagr.manager.Manager, timed_out: bool) -> None:
        """ This is called upon completion of evaluation (after manager.join()
        is complete). `timed_out` indicates if we reached a timeout. """

        # Print flag results
        if self.flags:
            for unit, flag in self.flags:
                print(f"Flag: {flag}")
        else:
            print("flag unable to be retrieved")
