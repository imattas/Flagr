...
from flagr import util
from flagr.unit import NotEnglishUnit, NotApplicable
...
def find_variables(text):
    ...
    beginning_pubkey = re.search(
        r"^-----BEGIN.*?-----\s", text, re.MULTILINE | re.DOTALL
    )
    ending_pubkey = re.search(r"-----END.*?-----\s*$", text, re.MULTILINE | re.DOTALL)
        ...
        def factor_n():
            ...
            if self.p == -1 and self.q == -1 and self.n != -1:
                ...
                if len(factors) == 2:
                    ...
                    raise NotImplementedError("factordb could not factor this!")
                else:
                    ...
                    # Grab the result and give it to Flagr
                    print(self.m)
