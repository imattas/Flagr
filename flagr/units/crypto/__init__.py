...
and can waste time for Flagr's operations.

For this reason, we implemented a commonly used 
:class:`flagr.units.crypto.CryptoUnit` that checks to ensure the target
...
class CryptoUnit(BaseUnit):
    """
    This Unit will raise :class:`flagr.unit.NotApplicable` if the
