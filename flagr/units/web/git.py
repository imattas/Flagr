...
This process is threaded, alongside Flagr already being threaded...
...
class DownloadWorker(Worker):
    ...
    def init(self, url, directory, retry, timeout, unit, flagr):
        self.session.verify = False
        self.session.mount(url, requests.adapters.HTTPAdapter(max_retries=retry))

    def do_task(self, filepath, url, directory, retry, timeout, unit, flagr):
        with closing(
            self.session.get(
                "%s/%s" % (url, filepath),
...
class FindObjectsWorker(DownloadWorker):
    ...
    def do_task(self, obj, url, directory, retry, timeout, unit, flagr):
...
def fetch_git(unit, url, directory, jobs, retry, timeout, flagr):
    """ 
    ...
    if (
        ...
        process_tasks(
            ...
            jobs,
            args=(url, directory, retry, timeout, unit, flagr),
        )
    ...
    process_tasks(
        tasks, DownloadWorker, jobs, args=(url, directory, retry, timeout, unit, flagr)
    )
    ...
    process_tasks(
        tasks, FindRefsWorker, jobs, args=(url, directory, retry, timeout, unit, flagr)
    )
