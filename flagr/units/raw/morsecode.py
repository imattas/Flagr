...
class Unit(RegexUnit):
    ...
    # This matches international or traditional morse code strings of at least 3 groups
    # Supports: dot-dash notation (.-, / as word separator), dit-dah notation
    PATTERN = re.compile(
        rb"((((dit|dah|di)-?)+)|([.\-]+))([\s/]((((dit|dah|di)-?)+)|([.\-]+))){2,}",
    ...
    def evaluate(self, match):
        ...
        # Split by multiple spaces or / for word boundaries
        words = re.split(rb"\s{2,}|/", morse)
        for word in words:
            word_result = []
            for letter in word.split(b" "):
                letter = letter.strip()
                if not letter:
                    continue
                if letter in international_morse_code_mapping:
                    word_result.append(international_morse_code_mapping[letter])
                elif letter in morse_alphabet:
                    word_result.append(morse_alphabet[letter])
            if word_result:
