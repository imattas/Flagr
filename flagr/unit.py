...
Units are the part of Flagr which actually performs the evaluation on a given target. They are defined as
subclasses of the ``flagr.unit.Unit`` class, and must implement three methods at a minimum to begin functioning.

Units are automatically loaded from the `flagr/units` directory, and optionally other directories specified at
...
class Unit(object):
    ...
    in order to be used with Flagr.
    ...
    Here's an example of a very basic unit class::
        ...
        class Unit(flagr.unit.Unit):
            ...
            def __init__(manager: flagr.manager.Manager, target: flagr.target.Target):
    ...
    def __init__(self, manager: flagr.manager.Manager, target: flagr.target.Target):
        """
        ...
        :param manager: The Flagr Manager instance
        ...
        self.target: flagr.manager.Target = target
    ...
    def can_recurse(self, unit_class: Type[flagr.unit.Unit]) -> bool:
    ...
    def enumerate(self):
        ...
        Flagr without further coding. By default, this method yields a single
...
class Finder(object):
    ...
    either within the default unit list bundled with Flagr or in a custom
    location.
    ...
    def __init__(self, manager: flagr.manager.Manager, use_default: bool = True):
        ...
        self.manager: flagr.manager.Manager = manager

        # Load default modules if requested
        if use_default:
            for unit in self.find(
                os.path.join(flagr.__path__[0], "units"), "flagr.units."
            ):
                try:
                    self.register(unit)
    ...
    def validate(self) -> None:
        ...
        if required (e.g. [flagr.units.crypto.caeser]) """
    ...
    def find(self, directory: str, prefix: str) -> Generator[Type[Unit], None, None]:
        """ Locate units which conform to the Flagr unit specification with
        ...
        class which subclasses ``flagr.unit.Unit`` and implements
        ...
        for importer, name, ispkg in pkgutil.walk_packages([directory], prefix):
            ...
            try:
                ...
                # Skip units with missing dependencies or other import errors
                continue
    ...
    def match(
        self, target: flagr.target.Target, scale: float = 1.0
        ...
        for unit_class in self.units:
            ...
            try:
                ...
                # - pure auto: no units are specified. auto is set, and flagr runs all applicable units.
                # - manual: units are specified and auto is not set. flagr runs all units within the specified set that
                #   are applicable.
