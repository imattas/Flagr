#!/usr/bin/env python3
""" A flagr manager which is capable managing the evaluation of arbitrary
...
# flagr imports
from flagr.target import Target, BadTarget
from flagr.unit import Unit, Finder
...
class Manager(configparser.ConfigParser):
    ...
    def set(self, section: str, option: str, value: Any = None) -> None:
        ...
        if option == "flag-format":
            # Convert user-friendly wildcard format to regex
            # * becomes .*? (match anything), | is kept as OR
            # e.g. "picoCTF{*}" becomes "picoCTF\{.*?\}"
            parts = value.split("|")
            pattern = "|".join(
                re.escape(part).replace(r"\*", ".*?") for part in parts
            )
            self.flag_pattern = re.compile(
                bytes(pattern, "utf-8"), re.IGNORECASE | re.MULTILINE | re.DOTALL
            )
        elif option == "threads":
            if self.running:
    ...
    def find_flag(self, unit: Unit, data: Any) -> bool:
        ...
        if match:
            ...
            try:
                ...
                if flagr.util.isprintable(found):
    ...
    def _prepare_results(self) -> None:
        ...
        from this run of flagr. This is automatically called when `start` is
