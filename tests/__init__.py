"""
Flagr Unit tests are designed to test the individual Flagr units. Confusingly,
we are also utilizing the Python UnitTest module. Each Python Unit Test should
correspond to a single Flagr Unit. The unit test will run Flagr with a target,
...
`FlagrTest`. Your tests should inherit from this class. The directory structure
for tests is the same as the `units` directory (e.g. `tests/units/category/unit`)

A Flagr Unit test must implement one or more `test_*` methods which will be
executed by the `unittest` module. The base `FlagrTest` module contains `setUp`
and `tearDown` methods to setup the Flagr monitor and manager objects. Inside
of the individual tests, you can call the `FlagrTest.flagr_test` method to
...
That's it! A Flagr Unit Test only requires that single call, however your test
...
from flagr.monitor import Monitor
from flagr.manager import Manager


class FlagrTest(TestCase):
    ...
    def setUp(self):
        """ Setup a Manager and Monitor object
            ...
            The Flagr Unit Tests can override this method, but must call the
    ...
    def flagr_test(
        ...
        :param config: Flagr configuration file
        :type config: str
        :param target: The target for this flagr test (url, filename, raw data, etc)
        :type target: str
        :param correct_flag: The correct flag which flagr should find.
