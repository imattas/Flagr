...
[![Run on Repl.it](https://repl.it/badge/github/JohnHammond/flagr)](https://repl.it/github/JohnHammond/flagr)
Flagr
==============
...
**Documentation:** [https://ctf-flagr.readthedocs.io](https://ctf-flagr.readthedocs.io)
...
[https://github.com/JohnHammond/ctf-flagr](https://github.com/JohnHammond/ctf-flagr)

Flagr is written in Python 3.
...
**Flagr will _automatically_ run code and do _potentially_ "malicious" things
...
After installation, Flagr will still require multiple external dependencies.
The installation of each of these depends on your distribution and package
manager, so an easier solution is to run Flagr through Docker. You can read
...
Whenever Flagr runs, it creates a `results` directory where it stores its 
findings and **artifacts** (files et. al.) that may be generated from units.

Flagr will not run if the `results` directory already exists. You can
have Flagr automatically remove the `results` directory before it runs with
...
Flagr works with a "boss -> worker" topology. One thread (the boss) spins off
...
To add functionality to Flagr, you simply need to create units. 
...
If you would like to contribute to Flagr, please see [`CONTRIBUTING.md`](CONTRIBUTING.md)
