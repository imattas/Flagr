...
When rewriting the Flagr framework, a lot of changes were made to the :class:`flagr.unit.Unit` interface. We tried
...
:class:`flagr.manager.Manager` class now takes the place of the old `Flagr` object. :class:`flagr.target.Target`
...
:func:`flagr.unit.Unit.check_deps` method, however this is almost never needed.
...
when interfacing with the :class:`flagr.unit.Finder` class and therefore also with the Manager's ``units``
...
mechanism is now in place. To modify recursion rules, you can now use the `flagr.unit.Unit.RECURSE_SELF`,
`flagr.unit.Unit.NO_RECURSE` and `flagr.unit.Unit.BLOCKED_GROUPS`.
...
The old data reporting mechanisms were part of the Flagr class. In the new framework, these were moved to
the :class:`flagr.manager.Manager` class and are all named as ``register_*``. For example, to register
...
:class:`flagr.monitor.Monitor`.

Generating and Reporting Artifacts
----------------------------------

Artifact creation used to be handled by the Flagr class, but has been moved to :class:`flagr.unit.Unit`.
However, the interface is largely the same for creating an artifact. To create an artifact, use the
:func:`flagr.unit.Unit.generate_artifact` method. The interface and parameters are the same as the old
`flagr.generate_artifact` method. The biggest difference is that the artifact will not automatically be
registered with the Manager or reported to the Monitor. To do that, call :func:`flagr.manager.Manager.register_artifact`.
