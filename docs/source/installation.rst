...
Flagr is designed first and foremost as a Python module. A ``setup.py`` script is provided to install via
``setuptools``. There are a number of binary dependencies which individual units depend on. When running Flagr, you
...
To install both the Flagr module and Read-Evaluate-Print-Loop (REPL) interpreter, use setup tools::

    python setup.py install

This will install Flagr and all of it's Python dependencies in your current environment.

External Unit Dependencies
--------------------------

On your first few runs of Flagr, you may find that you receive dependency errors related to binaries not present
on your system. These dependencies are specific to the units you have installed. The default units used by Flagr have
