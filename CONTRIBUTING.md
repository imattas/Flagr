...
If you do add code to Flagr, __we require documentation.__ We use simple 
...
organized exactly the same as the builtin units within Flagr. There is a 
`FlagrTest` class which inherits from UnitTest within the `tests` directory.
It provides a convenient function called `flagr_test` which makes it easy to
setup and run Flagr for a unit test. Ideally, create your test data on the 
